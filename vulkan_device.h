...
PINLINE bool create_device(vulkan_types* context)
{
        ...
        for (uint32_t phys_i = 0; phys_i != physical_device_count; ++phys_i) {
                ...
                if (phys_i == physical_device_index) {
                        context->physical_device_properties = device_properties;
                }
                ...
                for (u32 i = 0; i <  queue_family_count; i++) {
                        ...
                        VK_CHECK(vkGetPhysicalDeviceSurfaceSupportKHR(physical_devices[phys_i], i, context->surface, &supports_present));
                        if(supports_present){
                ...
                VK_CHECK(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physical_devices[phys_i], context->surface, &capabilities));
                VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(physical_devices[phys_i], context->surface, &format_count,0));
                VkSurfaceFormatKHR *formats = NULL;
                formats[format_count];
                VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(physical_devices[phys_i], context->surface, &format_count,formats));
                u32 present_mode_count;
                VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(physical_devices[phys_i], context->surface, &present_mode_count, 0));
                VkPresentModeKHR * present_modes = NULL;
                VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(physical_devices[phys_i], context->surface, &present_mode_count, present_modes));
        ...
        */
        printf("Transfer_family_index: %d\n Graphics_family_index: %d\n Compute_family_index: %d\nPresent_family_index: %d\n on gpu index: %d\n", context->transfer_family_index, context->graphics_family_index, context->compute_family_index, context->present_family_index,
               context->physical_device_index);
        
        context->gpu = physical_devices[physical_device_index];
	...
	if ( VK_EXT_subgroup_size_control)
	{
		...
		vkGetPhysicalDeviceProperties2 (context->gpu, &physical_device_properties_2);
		...
		vkGetPhysicalDeviceFeatures2 (context->gpu, &physical_device_features_2);
                
		//vulkan_physical_device_features = physical_device_features_2.features;
	}
	else
#endif
        vkGetPhysicalDeviceFeatures (context->gpu, &vulkan_physical_device_features);
