...
PINLINE bool create_swapchain(vulkan_types* context)
{
        
        uint32_t formatCount = 0;
        VkSurfaceFormatKHR surfaceFormats[20];
        VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(context->gpu, context->surface, &formatCount, 0));
        VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(context->gpu, context->surface, &formatCount, surfaceFormats));
		...
		{
			context->swap_format.colorSpace = format.colorSpace;
			context->swap_format.format = format.format;
                        break;
			
		}else{
                        context->swap_format =format;
                        break;
                        
                }
        }
        
        printf("Using surface format: %s for swapchain", vk_format_string(context->swap_format.format));
        
        VK_CHECK(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(context->gpu, context->surface, &context->surface_caps));
        VkExtent2D image_extent = context->surface_caps.currentExtent;
        if ( image_extent.width == 0xffffffff && image_extent.height == 0xffffffff ) {
		image_extent.width = MIN( context->surface_caps.maxImageExtent.width, MAX(context-> surface_caps.minImageExtent.width, (uint32_t) state.win_width ) );
		image_extent.height = MIN( context->surface_caps.maxImageExtent.height, MAX( context->surface_caps.minImageExtent.height, (uint32_t) state.win_height ) );
                
	}
        u32 presentModesCount;
        VkPresentModeKHR  presentModes[100];
        VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(context->gpu, context->surface, &presentModesCount, NULL));
        VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(context->gpu, context->surface, &presentModesCount, presentModes));
        ...
        for (uint32_t i = 0; i < presentModesCount; ++i)
	{
		...
		if (presentModes[i] == desiredMode)
		{
			...
			context->present_mode = desiredMode;
		}
	}
	...
	for (uint32_t i = 0; i < presentModesCount; ++i)
	{
		...
		{
			usedPresentMode = presentModes[i];
                        context->present_mode = usedPresentMode;
		}
	}
        
        
        
        printf("Using present mode: %s\n", pmode_to_str(context->present_mode) );
        u32 imageCount = 2;
        ...
        swapchain_info.imageFormat = context->swap_format.format;
        swapchain_info.imageColorSpace = context->swap_format.colorSpace;
        ...
        swapchain_info.preTransform = context->surface_caps.currentTransform;
        swapchain_info.compositeAlpha = VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR;
        swapchain_info.presentMode = context->present_mode;
        swapchain_info.clipped = VK_TRUE;
        swapchain_info.oldSwapchain = 0;
        VK_CHECK(vkCreateSwapchainKHR(context->logical_device, &swapchain_info, context->allocator, &context->swapchain));
