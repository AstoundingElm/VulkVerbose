...
static const char *GetDeviceVendorFromDriverProperties (VkPhysicalDeviceDriverProperties *driver_properties)
{
	switch ((int)driver_properties->driverID)
	{
		...
		return "VK_DRIVER_ID_GOOGLE_SWIFTSHADER/ VK_DRIVER_ID_GGP_PROPRIETARY";
...
const char* result_string =    vulkan_result_string(result, true);\
printf("Vulkan function failed with result:'%s',\n in file :%s,\n line:  %d\n", result_string, __FILE__, __LINE__);\
__builtin_trap();\
}\
}   
